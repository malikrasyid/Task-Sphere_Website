    ...
    <div class="p-5">
      <ul class="space-y-4">
        ...
        <li 
          ...
          class="bg-slate-50 rounded-lg p-4 hover:bg-gray-50 transition-colors shadow-sm"
        >
          <div class="font-medium text-gray-900 flex justify-between items-end">{{ item.name || item.title }}
            <span v-if="itemType === 'tasks'" class="px-2 py-0.5 text-white text-xs rounded-full" :style="{ backgroundColor: getTaskColor(item.status) }">
              {{ item.status }}
            </span>
          </div>
          ...
          <div v-if="itemType === 'tasks'" class="flex items-center text-xs text-gray-500 mt-2">
          ...
          <div v-if="itemType === 'projects'" class="flex items-center text-xs text-gray-500 mt-2">
            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
